...
function getAccurateMockData(domain) {
    const name = domain.split('.')[0];
    const tld = '.' + domain.split('.').slice(1).join('.');
    // Known taken domains WITH live sites
    const takenWithSites = {
        'workbrew.com': true,
        'google.com': true,
        'facebook.com': true,
        'tonyblount.com': true
...
async function checkPorkbunAvailability(domain) {
    console.log(`Checking Porkbun availability for: ${domain}`);
    const result = await porkbunRequest('/domain/check', {
        domain
    });
...
export async function checkMultipleDomains(baseName, tlds) {
    const results = {};
    const checks = await Promise.allSettled(tlds.map(tld => checkDomainAvailability(`${baseName}${tld}`)));
